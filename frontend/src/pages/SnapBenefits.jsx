...
export default function SnapBenefits() {
  ...
  useEffect(() => {
    ...
    async function loadSnapCenters() {
      try {
      ...
      >
        <div className="max-w-6xl mx-auto px-6 py-16">
          ...
          <h1 className="font-sans font-bold text-white text-4xl leading-tight mb-4 max-w-2xl">
            Apply for or renew SNAP benefits.
          </h1>
          <p className="text-cream/80 text-lg max-w-xl">
            Verified SNAP Center locations across NYC where you can apply, recertify, and get
            help with your case.
          </p>
        </div>
      </header>

      {/* SNAP center listings */}
      <section className="max-w-6xl mx-auto px-6 py-16">
        <div className="text-center mb-11">
          ...
          <h2 className="font-sans font-bold text-3xl text-charcoal mb-2.5">
            {loading
              ? "Loading..."
              : `${filteredResources.length} SNAP ${filteredResources.length === 1 ? "center" : "centers"} found`}
          </h2>
        ...
        {!loading && filteredResources.length === 0 && !error ? (
          <p className="text-center text-warm-gray text-sm">
            No SNAP centers matched your filter.
          </p>
        ) : (
