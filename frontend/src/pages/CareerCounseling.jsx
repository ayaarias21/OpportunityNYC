...
const CAREER_COUNSELING_KEYWORDS =
  /career guidance|career counsel|career assistance|vocational training|certification exam|job placement|job preparation/i;
...
export default function CareerCounseling() {
  ...
  useEffect(() => {
    ...
    async function loadResources() {
      try {
        const [workshopResponse, studentSupportResponse] = await Promise.all([
          getResources({ category: "Workshop", limit: 500 }),
          getResources({ category: "Student Support", limit: 500 }),
        ]);

        const combined = [...(workshopResponse.data || []), ...(studentSupportResponse.data || [])];
        const matches = combined.filter(
          (resource) =>
            CAREER_COUNSELING_KEYWORDS.test(resource.title) ||
            CAREER_COUNSELING_KEYWORDS.test(resource.description || "")
        );
        ...
        if (!cancelled) {
          setResources([]);
          setError(fetchError.message || "Could not load career counseling resources.");
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
      ...
      >
        <div className="max-w-6xl mx-auto px-6 py-16">
          ...
          <h1 className="font-sans font-bold text-white text-4xl leading-tight mb-4 max-w-2xl">
            Get guidance on your next career move.
          </h1>
          <p className="text-cream/80 text-lg max-w-xl">
            NYC partner organizations offering career guidance, vocational training, and
            certification support.
          </p>
        </div>
      </header>

      {/* Career counseling listings */}
      <section className="max-w-6xl mx-auto px-6 py-16">
        ...
        {!loading && resources.length === 0 && !error ? (
          <p className="text-center text-warm-gray text-sm">
            Career counseling resources will appear here after the NYC Open Data sync runs.
          </p>
        ) : (
